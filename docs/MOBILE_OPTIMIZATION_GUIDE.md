...
## 프로젝트에 이미 적용된 변경사항 (레포지토리 기준)

아래는 이 레포지토리에 실제로 적용된 모바일 관련 변경사항 요약입니다. 배너와 푸터 관련 CSS/컴포넌트가 조정되어 모바일에서 더 나은 레이아웃을 제공합니다.

- 배너 높이 및 스케일 조정: 모바일에서 배너의 높이를 줄이고 이미지 크기를 반응형으로 처리하도록 SCSS 수정.
- 푸터(Footer): '제품' 컬럼 제거, '회사' 컬럼의 링크를 헤더 메뉴와 동일한 내부 라우트(`/story`, `/products`, `/quality`, `/contact`)로 변경.
- 터치 타겟 확장: 푸터 링크의 패딩을 늘려 모바일에서 터치하기 편하도록 수정(최소 44x44 권장).

### 이미지 업로드 및 압축 권장사항

- Vercel의 업로드/함수 크기 제한을 고려하여 클라이언트 측에서의 이미지 압축을 권장합니다.
- 권장 최대 업로드 크기: 4.5MB 또는 그 이하 (Vercel 플랫폼 제한에 따라 변경 가능).
- 업로드 엔드포인트: `src/app/api/upload/route.ts` 를 확인하세요.
- 클라이언트에서 예: `browser-image-compression` 같은 라이브러리로 업로드 전에 이미지 크기와 해상도를 제한하세요.
